#!/usr/bin/env python3
"""Asserted Discord/Supabase auth patches for the generated v4.3 runtime."""
from __future__ import annotations


def _replace(source: str, search: str, replacement: str, label: str) -> str:
    if search not in source:
        raise SystemExit(f"Authentication integration failed ({label}).")
    return source.replace(search, replacement, 1)


def patch_auth_runtime(source: str) -> str:
    source = _replace(
        source,
...
''',
        ...
        while len(body) < content_length:
            ...
            body.extend(chunk)
        return request_parts[0].upper(), request_parts[1], headers, bytes(body[:content_length])
